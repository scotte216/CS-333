  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
          ...
          oldIntStat = SetInterruptsTo (DISABLED) -- Disabling interrupts
          
          --Check lock status
          if self.IsHeldByCurrentThread ()
             FatalError ("Current Thread already holds MUTEX lock!")
          
          -- If no one is holding the lock, acquire lock
          elseIf heldBy == null
            heldBy = currentThread
            
          -- Otherwise, the lock is held. Add to the wait list and sleep
          else
            waitingThreads.AddToEnd (currentThread)
            currentThread.Sleep ()
          endIf
      ...
      method Unlock ()
           ...
           if !self.IsHeldByCurrentThread ()
             FatalError ("Thread can't unlock a MUTEX it doesn't not own!")
           endIf
