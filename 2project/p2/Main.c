...
-----------------------------  SimpleThreadExample  ---------------------------------
  ...
  function SimpleThreadExample ()
    ...
    --    
      ...
      aThread.Fork (SimpleThreadFunction, 7)  -- Pass "4" as argument to the thread
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    emptySem: Semaphore = new Semaphore
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 6
        ...
        emptySem.Down () -- Semaphore to check if the array is full
                        -- Semaphore starts at BUFFER_SIZE so a SLEEP won't be
                        -- enacted until after the 5th instance of .Down() 
                        -- without any corresponding Up()
        producerLock.Lock () -- This locks the producers with a MUTEX

        -- Add c to the buffer
        buffer [bufferNextIn] = c
        bufferNextIn = (bufferNextIn + 1) % BUFFER_SIZE
        
        bufferLock.Lock() -- This locks the buffer since buffersize is used in 
                          -- elsewhere, including PrintBuffer
        ...
        bufferLock.Unlock() -- Unlocks the bufferLock
        
        -- Perform synchronization...
        fullSem.Up() -- Notifies the fullSem that a new character has 
                     -- been produced
        producerLock.Unlock() -- Unlocks the production MUTEX
      endFor
    endFunction
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
        fullSem.Down() -- Looks if the producers have added to 
                       -- the buffer
        consumerLock.Lock() -- Locks the thread from other consumers since they
                            -- have race conditions


        bufferLock.Lock() -- Add a MUTEX since BufferNextOut and BufferSize are used
                          -- by other functions (Producer and PrintBuffer)
        ...
        bufferLock.Unlock() -- unlocks the bufferlock MUTEX
        
        -- Perform synchronization...
        consumerLock.Unlock() -- Unlocks the Consumer function MUTEX
        emptySem.Up() -- Notifies the semaphore that a consumer has consumed
  ...
  function PrintBuffer (c: char)
    ...
    -- NOTE: MUTEX must be locked before calling this function due to shared
    -- variables: bufferNextOut and bufferSize
    --
      var
        i, j: int
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  enum HUNGRY, EATING, THINKING -- HUNGRY (1) EATING (2) THINKING (3)
  var
    mon: ForkMonitor
  ...
  class ForkMonitor
    superclass Object
    fields
      status: array [5] of int -- For each philosopher: HUNGRY(1), EATING(2), or THINKING(3)
      forkLock: Mutex
      monitor: array [5] of Condition
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
      PrintAllStatus ()
      Test(p: int,lock: ptr to Mutex)
  endClass
    ...
    method Init ()
      -- Initialize so that all philosophers are THINKING.
        var
          i: int
        forkLock = new Mutex
        monitor = new array of Condition {5 of new Condition}
 ...
 /*       forkLock.Lock ()
   ...
   */
        ...
        status[p] = HUNGRY
        self.PrintAllStatus ()
        self.Test(p,&forkLock)
        if status[p] != EATING
          monitor[p].Wait(&forkLock)
        endIf
        forkLock.Unlock()
      endMethod
